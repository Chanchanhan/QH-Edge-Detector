...
*******************************************************************************/
#ifndef RGB_CONVERT_MEX_HPP
#define RGB_CONVERT_MEX_HPP
...
//    getadd(I, I_data);
//    float *J_data = new float[I.rows * I.cols * I.channels()];
//    float norm = 1.0;
//    J_data = rgbConvert<float, float>(I_data, I.rows * I.cols, I.channels(), 2, norm);
...
Mat rgbToLuvu(Mat I) {
    ...
    getaddu(I, I_data);
    float *J_data = new float[I.rows * I.cols * I.channels()];
    for (int i = 0; i < I.rows * I.cols * I.channels(); i++) J_data[i] = 0;
    float norm = 1.0f / 255;

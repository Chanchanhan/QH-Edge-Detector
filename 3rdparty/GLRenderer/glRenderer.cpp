...
#define UNPROJECT_EPSILON 0.000001f
bool GLRenderer::unproject(float pixel_x, float pixel_y, float &X, float &Y, float &Z)
{
	GLdouble modelview[16];
	GLdouble projection[16];
	GLint winX, winY; GLfloat winZ;
	GLdouble posX, posY, posZ;

	glGetDoublev(GL_MODELVIEW_MATRIX, modelview);
	glGetDoublev(GL_PROJECTION_MATRIX, projection);

	GLint viewport[4] = { 0, 0, renderWidth, renderHeight };
#if 0 
	// freegult's viewport is not correct. width and height are reduced by 8 pixels.
	// so we set viewpoint manually
	glGetIntegerv(GL_VIEWPORT, viewport);
#endif

	winX = int(pixel_x);
	winY = viewport[3] - int(pixel_y) - 1;
	winZ = depthBuffer[winY*renderWidth + winX];

	int status = 1;
	if (abs(winZ - 1.0f) < UNPROJECT_EPSILON) // no intersection
	{
		status = 0;
		X = 0;
		Y = 0;
	...
	{
		status = gluUnProject(winX, winY, winZ, modelview, projection, viewport, &posX, &posY, &posZ);
		X = float(posX);
		Y = float(posY);
		Z = float(posZ);
	}	
...
void GLRenderer::getRGBABuffer()
{
	...
	{
		cv::Vec3b *rptr = rgbImg.ptr<cv::Vec3b>(renderHeight - i - 1);
...
int GLRenderer::initGLUT(int argc, char **argv)
{
	...
	glutInitWindowPosition(10, 10);                           // window location
	...
	Camera &cam, GLMmodel *mdl)
{
	...
	rgbImg = cv::Mat::ones(renderHeight, renderWidth, CV_8UC3);
...
void GLRenderer::initLights()
{
	...
	GLfloat lightKa[] = { 1, 1, 1, 1 };  // ambient light
...
void GLRenderer::dynamicChangeDrawMode()
{
	drawMode = drawMode % 3;
	if (drawMode == 0)        // fill mode
	{
